...
nStockTradeVec_SD_r2k_APYBT <- list();
conditionsMet_SD_r2k_APYBT <- list();
tradeMat_SD_r2k_APYBT <- list();
stockToAvoid2013_SD_r2k_APYBT <- list();
  ...
  for(stdMultiple in c(0.5,0.7,1)){
    ...
    for(nStocksBuy in c(1,4,8,10,20,60))
    {
      ...
      if(nStocksBuy == 10){
        ...
        stockToAvoid2013_SD_r2k_APYBT[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- stockToAvoid_yr;
      }
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_r2k_APYBT[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_r2k_APYBT[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_r2k_APYBT[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_r2k_APYBT, 
     conditionsMet_SD_r2k_APYBT,
     tradeMat_SD_r2k_APYBT,stockToAvoid2013_SD_r2k_APYBT,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_r2k_APYBT.Rdata' );
rm(nStockTradeVec_SD_r2k_APYBT,conditionsMet_SD_r2k_APYBT,tradeMat_SD_r2k_APYBT,stockToAvoid2013_SD_r2k_APYBT);
gc();
