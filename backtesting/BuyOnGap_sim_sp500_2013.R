#############################################################################
#
#    calculate for SP500 for year 2013 
#    
#   
#
#############################################################################
...
nStockTradeVec_SD_sp500_2013 <- list();
conditionsMet_SD_sp500_2013 <- list();
tradeMat_SD_sp500_2013 <- list();
...
for(stdLookback in c(132, 66, 44, 22, 10, 5 ))
{
  for(stdMultiple in c(0,0.5,0.7,1)){
    ...
    for(nStocksBuy in c(1,4,8,10,60))
    {
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_sp500_2013[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_sp500_2013[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_sp500_2013[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_sp500_2013, conditionsMet_SD_sp500_2013,tradeMat_SD_sp500_2013,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_sp500_2013.Rdata' );
rm(nStockTradeVec_SD_sp500_2013, conditionsMet_SD_sp500_2013,tradeMat_SD_sp500_2013);
gc();
