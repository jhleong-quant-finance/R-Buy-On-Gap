...
#
#    1) use previous yr performance to find trade to avoid
#    2) version 2 , added the rule for Gap to wider then 5%
#    
#    
#
#############################################################################
...
nStockTradeVec_SD_APYBTV3 <- list();
conditionsMet_SD_APYBTV3 <- list();
tradeMat_SD_APYBTV3 <- list();
stockToAvoid2013_SD_APYBTV3 <- list();

nskip <- 0
# 6 momths, 3 months, 2 months, 1 month, 2 weeks, 1 week
for(stdLookback in c(132, 66, 44)){
  for(stdMultiple in c(0.5,0.7,1,1.5,2)){
    ...
    for (i in 1:length(symbolsLst)) {
      ...
      # The num of tick must > stdLookback
    ...
    for(nStocksBuy in c(1,4,8,10,20,60))
    {
      ...
      # The Gap must wider then -2.5%
      ret_GT_SD <- ifelse(as.matrix(ret_GT_SD) < -0.05, ret_GT_SD, NA)
      ...
      if(nStocksBuy == 10){
        ...
        stockToAvoid2013_SD_APYBTV3[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- stockToAvoid_yr;
      }
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_APYBTV3[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_APYBTV3[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;        
      }
    ...
    nStockTradeVec_SD_APYBTV3[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_APYBTV3, 
     conditionsMet_SD_APYBTV3,
     tradeMat_SD_APYBTV3, stockToAvoid2013_SD_APYBTV3,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_APYBTV3.Rdata' );
rm(nStockTradeVec_SD_APYBTV3, conditionsMet_SD_APYBTV3,tradeMat_SD_APYBTV3);
gc();