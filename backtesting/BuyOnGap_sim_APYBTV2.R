...
#    2) version 2 , added the rule for Gap to wider then -2%
#    
#    
#
#############################################################################
...
nStockTradeVec_SD_APYBTV2 <- list();
conditionsMet_SD_APYBTV2 <- list();
tradeMat_SD_APYBTV2 <- list();
stockToAvoid2013_SD_APYBTV2 <- list();
  ...
  for(stdMultiple in c(0.5,0.7,1,1.5,2)){
    ...
    for(nStocksBuy in c(1,4,8,10,20,60))
    {
      ...
      if(nStocksBuy == 10){
        ...
        stockToAvoid2013_SD_APYBTV2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- stockToAvoid_yr;
      }
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_APYBTV2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_APYBTV2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;        
      }
    ...
    nStockTradeVec_SD_APYBTV2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_APYBTV2, 
     conditionsMet_SD_APYBTV2,
     tradeMat_SD_APYBTV2, stockToAvoid2013_SD_APYBTV2,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_APYBTV2.Rdata' );
rm(nStockTradeVec_SD_APYBTV2, conditionsMet_SD_APYBTV2,tradeMat_SD_APYBTV2);
gc();