...
getSymbols("^MID", env = indexData, src = "yahoo", from = startDate)
#Calculate returns for the index
indexRet <- (Cl(indexData$MID)-lag(Cl(indexData$MID),1))/lag(Cl(indexData$MID),1)
indexRet <- as.xts(indexRet)['2002::2012']
colnames(indexRet) <- "S&P400"

# function to make the stockData
# make_sp400_stockData()

nStockTradeVec_SD_sp400 <- list();
conditionsMet_SD_sp400 <- list();
tradeMat_SD_sp400 <- list();
...
for(stdLookback in c(132, 66, 44, 22, 10, 5 )){
  for(stdMultiple in c(0,0.5,0.7,1)){
    ...
    stockData <- new.env();
    gc();
    load_sp400_stockData(stockData);
    ...
    for(nStocksBuy in c(1,2,4,5,8,10,20,40,60))
    {
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_sp400[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_sp400[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_sp400[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_sp400, conditionsMet_SD_sp400,tradeMat_SD_sp400,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_sp400.Rdata' );
rm(nStockTradeVec_SD_sp400, conditionsMet_SD_sp400,tradeMat_SD_sp400);
gc();