...
nStockTradeVec_SD_r2k <- list();
conditionsMet_SD_r2k <- list();
tradeMat_SD_r2k <- list();
...
for(stdLookback in c(132, 66, 44)){
  for(stdMultiple in c(0.5,0.7,1)){
    ...
    for(nStocksBuy in c(1,4,8,10,20,60))
    {
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_r2k[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_r2k[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_r2k[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_r2k, conditionsMet_SD_r2k,tradeMat_SD_r2k,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_r2k.Rdata' );
rm(nStockTradeVec_SD_r2k, conditionsMet_SD_r2k,tradeMat_SD_r2k);
gc();
