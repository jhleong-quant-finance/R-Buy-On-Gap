...
#
#    S&P500
#    calculate for diff period for SD. 
#    
#    ? shorten the period for SD might take care of the near term volatility.
#    ? want to take into account of the stock in near term down trend.
#    ? need a bigger gap for such stock.
#
#############################################################################
library("quantmod")
...
nStockTradeVec_SD_sp500 <- list();
conditionsMet_SD_sp500 <- list();
tradeMat_SD_sp500 <- list();
...
for(stdLookback in c(132, 66, 44)){
  for(stdMultiple in c(0.5,0.7,1,1.5,2)){
    ...
    for(nStocksBuy in c(1,4,8,10,20,60))
    {
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_sp500[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_sp500[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_sp500[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_sp500, conditionsMet_SD_sp500,tradeMat_SD_sp500,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_sp500.Rdata' );
rm(nStockTradeVec_SD_sp500, conditionsMet_SD_sp500,tradeMat_SD_sp500);
gc();