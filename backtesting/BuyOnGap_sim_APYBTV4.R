...
#    2) version 2 , added the rule for Gap to wider then 1%
#    
#    
#
#############################################################################
...
nStockTradeVec_SD_APYBTV4 <- list();
conditionsMet_SD_APYBTV4 <- list();
tradeMat_SD_APYBTV4 <- list();
stockToAvoid2013_SD_APYBTV4 <- list();
  ...
  for(stdMultiple in c(0.5,0.7,1,1.5,2)){
    ...
    for(nStocksBuy in c(1,4,8,10,20,60))
    {
      ...
      if(nStocksBuy == 10){
        ...
        stockToAvoid2013_SD_APYBTV4[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- stockToAvoid_yr;
      }
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_APYBTV4[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_APYBTV4[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;        
      }
    ...
    nStockTradeVec_SD_APYBTV4[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_APYBTV4, 
     conditionsMet_SD_APYBTV4,
     tradeMat_SD_APYBTV4, stockToAvoid2013_SD_APYBTV4,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_APYBTV4.Rdata' );
rm(nStockTradeVec_SD_APYBTV4, conditionsMet_SD_APYBTV4,tradeMat_SD_APYBTV4);
gc();