...
# function to make the stockData
# make_sp500_stockData();

nskip <- 0;

nStockTradeVec_SD_2day <- list();
conditionsMet_SD_2day <- list();
tradeMat_SD_2day <- list();
  ...
  for(stdMultiple in c(0,0.5,0.7,1)){
    ...
    for (i in 1:length(symbolsLst)) {
      ...
      if(nrow(sData) >= stdLookback)
      {
        ...
        ####### the trade buy at open and sell on tomolo close. ########
        
        dayClOpRet <- (lag(Cl(sData), k=-1)-Op(sData))/Op(sData)
        ...
        colnames(sData) <- oldColNames
        eval(parse(text=paste("stockData$",symbolsLst[i]," <- cbind(sData,lowOpenRet,stdClClRet,dayClOpRet, sd, mean)",sep="")))
      }
      else
      {
    ...
    for(nStocksBuy in c(1,2,4,5,8,10,20,40,60))
    {
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_2day[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_2day[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_2day[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_2day, conditionsMet_SD_2day, tradeMat_SD_2day,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_2day.Rdata' );
rm(nStockTradeVec_SD_2day, conditionsMet_SD_2day, tradeMat_SD_2day);
gc();