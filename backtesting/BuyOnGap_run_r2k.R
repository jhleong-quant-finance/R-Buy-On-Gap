#############################################################################
#
#    Implementation for Buy On Gap strategy for Russell 2000
#
#############################################################################
...
nStockTradeVec_run_r2k <- list();
conditionsMet_run_r2k <- list();
tradeMat_run_r2k <- list();

nskip <- 0

# 6 momths, 3 months, 2 months, 1 month, 2 weeks, 1 week
for(stdLookback in c(66)){
  for(stdMultiple in c(0.5)){
    ...
    for (i in 1:length(symbolsLst)) {
      ...
      sData <- last(sData, '4 months') # at least more then 3 months of data for stdLookback = 66 "3 momths"
    ...
    for(nStocksBuy in c(10))
    {
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_run_r2k[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_run_r2k[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_run_r2k[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_run_r2k, conditionsMet_run_r2k,tradeMat_run_r2k,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_run_r2k.Rdata' );
rm(nStockTradeVec_run_r2k, conditionsMet_run_r2k,tradeMat_run_r2k);
gc();
