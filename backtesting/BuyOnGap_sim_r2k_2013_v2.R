#############################################################################
#
#    calculate for Russell 2000 for year 2013 version 2
#    1. Daily Vol >= 500K
...
nStockTradeVec_SD_r2k_2013_v2 <- list();
conditionsMet_SD_r2k_2013_v2 <- list();
tradeMat_SD_r2k_2013_v2 <- list();

low_vol_stock <- function()
{
  load(file="/data/finance/US/US_stock_fundmental_data.Rdata");
  low_vol_stock <- cl_funddata[cl_funddata$'Ave. Daily Volume' < 500000, 'Symbol'];
...
for(stdLookback in c(66)){
  for(stdMultiple in c(0.5,1)){
    ...
    for(nStocksBuy in c(1,4,8,10,60))
    {
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_r2k_2013_v2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_r2k_2013_v2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_r2k_2013_v2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_r2k_2013_v2, conditionsMet_SD_r2k_2013_v2,tradeMat_SD_r2k_2013_v2,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_r2k_2013_v2.Rdata' );
rm(nStockTradeVec_SD_r2k_2013_v2, conditionsMet_SD_r2k_2013_v2,tradeMat_SD_r2k_2013_v2);
gc();
