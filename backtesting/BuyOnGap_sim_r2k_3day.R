...
nStockTradeVec_SD_r2k_3day <- list();
conditionsMet_SD_r2k_3day <- list();
tradeMat_SD_r2k_3day <- list();
...
for(stdLookback in c(66)){
  for(stdMultiple in c(0.5,1)){
    ...
    for (i in 1:length(symbolsLst)) {
      ...
      if(nrow(sData) >= stdLookback)
      {
        ...
        colnames(stdClClRet) <- paste(symbolsLst[i],".StdClClRet",sep="")
	      ...
	      ####### the trade buy at open and sell on 2 days later. ########

        dayClOpRet <- (lag(Cl(sData), k=-2)-Op(sData))/Op(sData)
    ...
    for(nStocksBuy in c(1,4,8,10,60))
    {
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_r2k_3day[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_r2k_3day[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_r2k_3day[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_r2k_3day, conditionsMet_SD_r2k_3day,tradeMat_SD_r2k_3day,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_r2k_3day.Rdata' );
rm(nStockTradeVec_SD_r2k_3day, conditionsMet_SD_r2k_3day,tradeMat_SD_r2k_3day);
gc();
