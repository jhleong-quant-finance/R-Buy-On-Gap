...
#    
#     1) The Gap must be wider then 5%
#
#############################################################################
...
nStockTradeVec_SD_r2k_APYBT_V2 <- list();
conditionsMet_SD_r2k_APYBT_V2 <- list();
tradeMat_SD_r2k_APYBT_V2 <- list();
stockToAvoid2013_SD_r2k_APYBT_V2 <- list();
  ...
  for(stdMultiple in c(0.5,0.7,1)){
    ...
    for(nStocksBuy in c(1,4,8,10,20,60))
    {
      ...
      if(nStocksBuy == 10){
        ...
        stockToAvoid2013_SD_r2k_APYBT_V2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- stockToAvoid_yr;
      }
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_r2k_APYBT_V2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_r2k_APYBT_V2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_r2k_APYBT_V2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_r2k_APYBT_V2, 
     conditionsMet_SD_r2k_APYBT_V2,
     tradeMat_SD_r2k_APYBT_V2, stockToAvoid2013_SD_r2k_APYBT_V2,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_r2k_APYBT_V2.Rdata' );
rm(nStockTradeVec_SD_r2k_APYBT_V2,conditionsMet_SD_r2k_APYBT_V2,tradeMat_SD_r2k_APYBT_V2);
gc();
