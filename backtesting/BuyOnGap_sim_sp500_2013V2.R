#############################################################################
#
#    calculate for SP500 for year 2013 version 2
#    
#    condition include the Gap must be > -5%
#
#############################################################################
library("quantmod")
...
nStockTradeVec_SD_sp500_2013V2 <- list();
conditionsMet_SD_sp500_2013V2 <- list();
tradeMat_SD_sp500_2013V2 <- list();
...
for(stdLookback in c(132, 66, 44, 22, 10, 5 ))
{
  for(stdMultiple in c(0,0.5,0.7,1)){
    ...
    #cat("Calculating the returns and standard deviations for stock: ");
    for (i in 1:length(symbolsLst)) {
      # print processing stock name
    #  cat(symbolsLst[i]);cat(' ');
  ...
  #  cat("\n");
   ...
   # cat("Processing return table for stock: ");
    for (i in 1:length(symbolsLst)) {
      # print processing stock name
    #  cat(symbolsLst[i]);cat(' ');
  ...
  #  cat('\n');
    ...
    for(nStocksBuy in c(1,4,8,10,60))
    {
      ...
      ################### Gap > 5% ######################
      # exlcude retMat with positive ret and the Gap must bigger then 5%
      ret_GT_SD <- ifelse(as.matrix(ret_GT_SD) < -0.02, ret_GT_SD, NA)
      ################### Gap > 5% ######################
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_sp500_2013V2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_sp500_2013V2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_sp500_2013V2[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_sp500_2013V2, conditionsMet_SD_sp500_2013V2,tradeMat_SD_sp500_2013V2,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_sp500_2013V2.Rdata' );
rm(nStockTradeVec_SD_sp500_2013V2, conditionsMet_SD_sp500_2013V2,tradeMat_SD_sp500_2013V2);
gc();