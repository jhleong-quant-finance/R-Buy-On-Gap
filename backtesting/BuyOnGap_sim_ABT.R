...
#
#
#    find stocks with significant frequent appearance with losing trade. 
#    
#    make a trading rule to avoid trading those stocks.
#    result increase performance across the yr except 2003
#
#
#############################################################################

load(file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_skip.Rdata' );
source('/home/jhleong/dev/R/lib/libtrade.R');
numTradePerStock2 <- buy_on_gap.numTradePerStock((conditionsMet_SD_skip[['0']][['0.5']])['2003::2012'], (tradeMat_SD_skip[['0']][['0.5']])['2003::2012'])

# Top 25%
topQuarter <- floor(nrow(numTradePerStock2[numTradePerStock2$Count > 0,])/4)
numTradePerStock_top2 <- numTradePerStock2[1:topQuarter,]
#numTradePerStock_top2[order(-numTradePerStock_top2$MeanRet),]

StockToAvoid <- subset(numTradePerStock_top2, MeanRet <= 0, select = Symbol)
...
colnames(indexRet) <- "S&P500"


nStockTradeVec_SD_ABT <- list();
conditionsMet_SD_ABT <- list();
tradeMat_SD_ABT <- list();
...
for(stdLookback in c(132, 66, 44, 22, 10, 5 )){
  for(stdMultiple in c(0,0.5,0.7,1)){
    ...
    stockData <- new.env();
    gc();
    load_sp500_stockData(stockData);
    ...
    symbolsLst <- subset(symbolsLst, !symbolsLst %in% StockToAvoid)
    symToExclude <- vector();
    ...
    for(nStocksBuy in c(1,2,4,5,8,10,20,40,60))
    {
      ...
      print(paste('Processed nskip=',nskip,' SD=',stdMultiple ,' nStocksBuy=', nStocksBuy, sep=''))
      
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_ABT[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_ABT[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_ABT[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_ABT, conditionsMet_SD_ABT,tradeMat_SD_ABT,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_ABT.Rdata' );
rm(nStockTradeVec_SD_ABT, conditionsMet_SD_ABT,tradeMat_SD_ABT);
gc();