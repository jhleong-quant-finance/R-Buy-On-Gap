...
nStockTradeVec_SD_sp600 <- list();
conditionsMet_SD_sp600 <- list();
tradeMat_SD_sp600 <- list();
  ...
  for(stdMultiple in c(0.5,0.7,1,1.5,2)){
    ...
    for(nStocksBuy in c(1,4,8,10,20,60))
    {
      ...
      for (i in 1:length(ret_GT_SD[,1])){
        orderindex <- order((ret_GT_SD[i,]),decreasing=FALSE, na.last=NA)
        
        ################### skip the n most negative return ######################
        
        orderindex <- orderindex[(nskip+1):(nStocksBuy+nskip)] 
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_sp600[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_sp600[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;
      }
    ...
    nStockTradeVec_SD_sp600[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_sp600, conditionsMet_SD_sp600,tradeMat_SD_sp600,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_sp600.Rdata' );
rm(nStockTradeVec_SD_sp600, conditionsMet_SD_sp600,tradeMat_SD_sp600);
gc();