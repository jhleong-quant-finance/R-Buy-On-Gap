...
#    result: improvement on all years except 2008, return improve 20-40%
#            improvement is mixed with compare to previous test with StockToAvoid using 10 yr of data
#    
#
#############################################################################
library("quantmod")
...
nStockTradeVec_SD_APYBT <- list();
conditionsMet_SD_APYBT <- list();
tradeMat_SD_APYBT <- list();
stockToAvoid2013_SD_APYBT <- list();
  ...
  for(stdMultiple in c(0.5,0.7,1,1.5,2)){
    ...
    for(nStocksBuy in c(1,4,8,10,20,60))
    {
      ...
      if(nStocksBuy == 10){
        ...
        stockToAvoid2013_SD_APYBT[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- stockToAvoid_yr;
      }
      ...
      if(nStocksBuy == 10)
      {
        conditionsMet_SD_APYBT[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- conditionsMet;
        tradeMat_SD_APYBT[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- tradeMat;        
      }
    ...
    nStockTradeVec_SD_APYBT[[as.character(nskip)]][[as.character(stdLookback)]][[as.character(stdMultiple)]] <- nStockTradeVec
  }
}

save(nStockTradeVec_SD_APYBT, 
     conditionsMet_SD_APYBT,
     tradeMat_SD_APYBT, stockToAvoid2013_SD_APYBT,
     file='/home/jhleong/dev/R/buy_on_gap/result_Rdata/nStockTradeVec_SD_APYBT.Rdata' );
rm(nStockTradeVec_SD_APYBT, conditionsMet_SD_APYBT,tradeMat_SD_APYBT);
gc();